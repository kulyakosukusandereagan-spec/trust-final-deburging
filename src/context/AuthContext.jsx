...
export const AuthContext = createContext({
  ...
  loading: true,
  isOnline: true,
  setShopId: async (newShopId) => {},
  logout: async () => {},
  showShopModal: false,
  setShowShopModal: (show) => {}
});
...
export function AuthProvider({ children }) {
  ...
  const [shopId, setShopIdState] = useState('junub-main-001');
  const [loading, setLoading] = useState(true);
  const [isOnline, setIsOnline] = useState(typeof navigator !== 'undefined' ? navigator.onLine : true);
  const [showShopModal, setShowShopModal] = useState(false);
  const [inputShopCode, setInputShopCode] = useState('junub-main-001');
  ...
  const setShopId = async (newShopId) => {
    ...
    if (currentUser) {
      try {
        ...
        setUserDoc(prev => ({ ...prev, shopId: cleanId }));
      } catch (err) {
